// One opt-in v8 production-chain sample. No prompt edits, rerolls or player-save access.
// Offline preflight by default; live: node scripts/assess-airp-scoped-v8.mjs --live <expected calls>
// Optional --after <ready report directory>: replay prior real outputs, then generate only the next step.
...
assert(process.argv.length === 2 || live && [4, 6].includes(process.argv.length) && /^\d+$/.test(process.argv[3]) && (process.argv.length === 4 || process.argv[4] === '--after'));
const expected = live ? Number(process.argv[3]) : null, allowance = 4, root = process.cwd();
const parentDirectory = process.argv[5] ? path.resolve(process.argv[5]) : null;
...
try {
  ...
  material.models = runtime.resolveGenerationModels(config.models, material.preset, 8);
  assert.equal(material.resources.version, 8);
  ...
  const f = poolTestRuntime(undefined, 'outline-v8-elora');
  assert((await f.runtime.application.create({protocolVersion: 4, contentVersion: 19, profileId: 'profile.demo.first-run', saveId: 'outline-v8-elora', epoch: 'outline-v8-test', clientRequestId: 'create-outline-v8'})).ok);
  ...
  const historyReports = [], seen = new Set();
  for (let previous = parentDirectory; previous;) {
    assert(previous.startsWith(path.join(root, 'dist/reports/airp-outline-v8-live-')) && !seen.has(previous), 'Invalid/cyclic prior report');
    seen.add(previous); assert(seen.size <= 5, 'Too many prior steps');
    const prior = JSON.parse(await fs.readFile(path.join(previous, 'preflight.json'), 'utf8'));
    const summary = JSON.parse(await fs.readFile(path.join(previous, 'summary.json'), 'utf8'));
    assert.equal(summary.status, 'ready', 'Only successful real output can become read history');
    assert.equal(prior.resourcesHash, hash(material.resources), 'Prompt revision changed; do not silently replay with new prompts');
    assert.equal(prior.presetHash, hash(material.preset), 'Preset revision changed');
    historyReports.unshift({directory: previous, prior, summary}); previous = prior.parentDirectory ?? null;
  }
  for (const history of historyReports) {
    let priorJob = await current(); assert.equal(priorJob.scene.role, history.prior.role);
    for (const a of history.summary.attempts) {
      const saved = JSON.parse(await fs.readFile(path.join(history.directory, `${a.stage}-${a.ordinal}.json`), 'utf8'));
      const repair = a.stage === 'formatting' && a.ordinal === 2 ? {formatRepair: 1} : {};
      assert.equal(sanitize(compileDirectorJob(material, priorJob, repair.formatRepair)), sanitize(saved.input), 'Frozen prior input differs');
      const attemptId = `${a.stage}-${a.ordinal}`;
      await f.send({type: 'airp-director-begin', jobId: priorJob.id, attemptId, stage: a.stage, at: saved.startedAt, ...repair});
      await f.send({type: 'airp-director-result', jobId: priorJob.id, attemptId, at: saved.endedAt, output: saved.text, usage: saved.usage});
      priorJob = await current(); assert.equal(priorJob.attempts.at(-1).status, a.status);
    }
    assert(priorJob.text, 'Missing replayed Chinese scene');
    await f.send({type: 'airp-director-show', jobId: priorJob.id});
    for (let cursor = 0; cursor < priorJob.text.lines.length; cursor++) await f.send({type: 'airp-director-read', jobId: priorJob.id, cursor});
    if (['offer', 'action'].includes(priorJob.scene.role)) await f.send({type: 'airp-director-choose', eventId, choiceId: 'participate'});
    await f.send({type: 'airp-director-open', eventId});
  }
  let job = await current(); assert(!job.text, 'Event has no next unread step');
  ...
  const preflightJob = {...job, attempts: [{id: 'preflight', stage: 'planning', ordinal: 1, inputHash: '', at: 1, endedAt: 2,
    status: 'succeeded', output: planningPreflight(8), usage: emptyUsage(), outcomeUnknown: false, error: null}]};
  ...
  const preflight = {
    version: 8, title: job.scene.card.title, role: job.scene.role, actorIds: job.scene.actorIds, parentDirectory,
  ...
  if (!live) {
    ...
    directory = await fs.mkdtemp(path.join(root, 'dist/reports/airp-outline-v8-live-')); await fs.chmod(directory, 0o700);
    ...
    for (const stage of ['planning', 'writing', 'formatting']) {
      ...
      for (let ordinal = 1; ordinal <= (stage === 'formatting' ? 2 : 1); ordinal++) {
        ...
        const provider = createDirectProvider(async (url, init) => {
          ...
          await save(`${stem}-request.json`, JSON.parse(init.body));
        ...
        try {
          ...
          if (attempt.status !== 'succeeded') {
            ...
            try {
              if (stage === 'planning') validateCreativeStage(stage, completion.text, '', actors, 8);
              else if (stage === 'writing') validateDirectorWriting(completion.text, material, job);
              else acceptDirectorText(completion.text, directorProse(material, job), job.scene.actorIds, 8);
    ...
    if (job.text) {
      ...
      restored = (await poolTestRuntime().runtime.application.restoreSave({archive: JSON.stringify({archiveVersion: 4, record}), clientRequestId: 'v8-live-restore'})).ok;
    ...
    const eventState = (await f.read()).airpDirector.events.find(e => e.id === eventId);
    if (job.text && job.scene.role === 'result') assert.equal(eventState.status, 'resolved', 'Reading the result did not resolve the event');
    const summary = {version: 8, directory, title: preflight.title, role: preflight.role, parentDirectory, status: job.text ? 'ready' : 'failed', eventStatus: eventState.status, readScenes: eventState.readSceneIds.length, startingCalls: expected, calls: ledger.calls, limit: ledger.limit,
    ...
    const transcript = ['# AIRP v8 · 三模型完整输出', '', `场景：${preflight.title}／${preflight.role}；玩家显示名：林恩。`, '',
    ...
    if (parentDirectory) transcript.push(`前文来自：${parentDirectory}；复用已通过的真实输出，隔离测试通过正式指令选择“参与”，没有重调前文模型。`, '');
    for (const stage of ['planning', 'writing', 'formatting']) {

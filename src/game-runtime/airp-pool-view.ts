...
export function airpPoolView(content: AirpPoolContent, state: AirpPoolState, record: D5GameRecord) {
  ...
  const entries = state.instances.map(instance => {
    ...
    const history = state.scenes.filter(s => s.instanceId === instance.id && !["departure", "found"].includes(s.role) && !airpOnlineHidden(record.airpOnline?.entries.find(e => e.sceneId === s.id))).map(scene => {

...
export function validateManorContent(catalog: DemoCatalog, ordinaryRoutes = false) {
  ...
  if (first.id === maintenance.id || !ordinaryRoutes && Object.keys(catalog.routes).length !== 2) v.invalid("manor.routes", "Two distinct routes required");
  ...
  v.choice(reward.id, ["reward.old-manor.first-clear"], "reward.id"); v.number(reward.gold, "reward.gold", 0, catalog.contentVersion >= 17 ? 100_000 : 1000);

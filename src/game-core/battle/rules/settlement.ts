...
export function getLayerSettlement(context: BattleContext,
  state: ExpeditionState,
  ...
  const payout = getLayerPayout(context, state);
  return {
    layer: state.layer,
    round: state.round,
    baseGold: state.gold,
...
function bankLayerEffects(context: BattleContext,
  state: ExpeditionState,
  closingHand: HandEvaluation | null
): { settlement: LayerSettlement; effects: AtomicEffect[] } {
  const settlement = getLayerSettlement(context, state, closingHand);
...
export function settleEnemyTurnTransition(context: BattleContext,
  state: ExpeditionState,
  rng: Rng,
  closingHand: HandEvaluation | null
): BattleTransition {
  ...
  if (activeParty.length === 0) {
    ...
    const banked = bankLayerEffects(context, state, closingHand);
    effects.push(...banked.effects);

    if (state.layer >= context.catalog.balance.MAX_LAYER) {
      effects.push(
        logEffect(
          state,
          "leave-log",
...
export function leaveExpeditionTransition(context: BattleContext,
  state: ExpeditionState,
  rng: Rng
): BattleTransition {
  ...
  const multiplier = roundTwo(1 + state.handMultiplier);
  const baseGold = state.bagGold;
  const pendingPayout = getLayerPayout(context, state);
  const effects: AtomicEffect[] = [];
  let totalGold = state.bagGold;
  if (pendingPayout > 0) {
    const banked = bankLayerEffects(context, state, null);
    effects.push(...banked.effects);
    totalGold = banked.settlement.bagAfter;
  }
  effects.push(

...
export function performInitialRoll(context: BattleContext,
  state: ExpeditionState,
  rng: Rng
): BattleTransition {
  ...
  return finishResolution(state, resolveAtomicEffects(context, state, effects));
}

/** Reroll every eligible unloaded die and auto-load the final result set. */
export function performReroll(context: BattleContext,
  state: ExpeditionState,
  rng: Rng
): BattleTransition {
...
export function performToggleLoad(context: BattleContext,
  state: ExpeditionState,
  dieIndex: number
): BattleTransition {
  ...
  const action = loaded
    ? `装载${context.catalog.characters[die.ownerId].name}的骰子`
    : `卸载${context.catalog.characters[die.ownerId].name}的骰子`;
  ...
  const resolved = resolveEffectsCommand(context, state, action, [effect]);
  return resolved.error
    ? failed(state)
    : { state: resolved.state, events: resolved.events, error: null };
}

...
import { createSummonedEnemy } from "../rules/enemies";
...
function resolveAttack(context: BattleContext,
  state: ExpeditionState,
  enemy: EnemyState,
  ...
  effects.push(
    damage <= 0
      ? logEffect(
          ...
          "blocked-log",
          "good",
          `${enemy.name}对${context.catalog.characters[target.id].name}的攻击被完全挡下。`
        )
      : logEffect(
          state,
          enemy,
          "damage-log",
          "bad",
          `${context.catalog.characters[target.id].name}受到 ${damage} 点伤害。`
        )
  );

  if (lethal) {
    const rustAdded = target.rustLevel < getRustableFaceCapacity(context, target.id);
    effects.push(
      logEffect(
        state,
        enemy,
        "downed-log",
        "bad",
        `${context.catalog.characters[target.id].name}力竭倒下，${
          rustAdded ? "命数新增 1 个锈面" : "命数已完全锈蚀"
        }；本层余下战斗无法行动，进入下一层时将以 ${context.catalog.balance.DOWNED_RETURN_HP} 点生命归队。`
      ),
      factEffect(
        state,
        enemy,
        "downed-fact",
        `${context.catalog.characters[target.id].name}被${enemy.name}击倒，命数因伤势进一步锈蚀。`
      )
    );
  }
  if (enemy.kind === "charger") {
...
function resolveSeal(context: BattleContext,
  state: ExpeditionState,
  enemy: EnemyState,
  ...
  if (target && !target.downed) {
    effects.push(
      ...
      logEffect(
        ...
        "seal-log",
        "bad",
        `${enemy.name}封锁了${context.catalog.characters[target.id].name}的下一回合骰子。`
      )
    );
  }
  effects.push(cancelIntentEffect(state, enemy));
...
function resolveSummon(context: BattleContext,
  state: ExpeditionState,
  enemy: EnemyState,
  ...
  if (state.enemies.filter((candidate) => !isEnemyDefeated(candidate)).length < 5) {
    ...
    const spawned = createSummonedEnemy(context, () => enemySequence);
    effects.push(
      {
      ...
      logEffect(
        ...
        "summon-log",
        "bad",
        `${enemy.name}召唤了一只 ${spawned.maxHp} 血${spawned.name}。`
      )
    );
  }
  effects.push(cancelIntentEffect(state, enemy));
...
function buildIntentEffects(context: BattleContext,
  state: ExpeditionState,
  enemy: EnemyState,
  intent: EnemyIntent
): { event: EnemyTurnEvent; effects: AtomicEffect[] } {
  switch (intent.type) {
    case "attack":
      return resolveAttack(context, state, enemy, intent);
    case "charge":
      return resolveCharge(state, enemy, intent);
    case "seal":
      return resolveSeal(context, state, enemy, intent);
    case "countdown":
      return resolveCountdown(state, enemy, intent);
    case "summon":
      return resolveSummon(context, state, enemy, intent);
  }
...
export function resolveEnemyIntentTransition(context: BattleContext,
  state: ExpeditionState,
  enemyId: string
): EnemyIntentTransition {
  ...
  const { event, effects } = buildIntentEffects(context, state, enemy, enemy.intent);
...
export function resolveEnemyTurnStep(context: BattleContext,
  state: ExpeditionState,
  enemyId: string
): EnemyTurnStep {

...
function buildStallEffects(context: BattleContext,
  state: ExpeditionState
): { effects: AtomicEffect[]; fledIds: Set<string> } {
  const effects: AtomicEffect[] = [];
  const fledIds = new Set<string>();
  if (!assessStalling(context, state).stalling) return { effects, fledIds };
  ...
  for (const enemy of state.enemies) {
    ...
    if (
      !isEnemyFrenzied(context, state, enemy) &&
      !hasEnemyFrenzyWarning(context, state, enemy) &&
      enemy.attack > 0
    ) {
      effects.push(
        {
          ...
          status: {
            ...
            definitionId: context.catalog.frenzyWarningId,
            sourceId: enemy.id,
            stacks: 1,
            maxStacks: 1,
...
export function prepareEnemyTurnTransition(context: BattleContext,
  state: ExpeditionState
  ...
  const hand = evaluateHand(context, state);
  ...
  const stall = buildStallEffects(context, projected);
  ...
  const resolved = resolveAtomicEffects(context, state, effects);
  if (resolved.error) {
    return {
      state,
      events: [],
...
export function prepareEnemyTurn(context: BattleContext, state: ExpeditionState): PreparedEnemyTurn {
  const transition = prepareEnemyTurnTransition(context, state);
...
export function applyFrenzyRecoilTransition(context: BattleContext,
  state: ExpeditionState
): BattleTransition {
  ...
  const effects: AtomicEffect[] = [];
  for (const enemy of state.enemies) {
    if (isEnemyDefeated(enemy) || !isEnemyFrenzied(context, state, enemy)) continue;
    effects.push(
      {
...
export function activateScheduledFrenzyForNextRoundTransition(context: BattleContext,
  state: ExpeditionState
): BattleTransition {
  if (state.mode.type !== "enemy-turn") {
    return { state, events: [], error: "command-not-available" };
    ...
    effects.push(
        ...
        : []),
      {
        ...
        status: {
          ...
          definitionId: context.catalog.frenzyActiveId,
          sourceId: enemy.id,
          stacks: 1,
          maxStacks: 1,
          duration: null,
      ...
      },
      {
        ...
        stat: "attack",
        operation: "add",
        value: context.catalog.balance.FRENZY_ATTACK_BONUS
      },
      enemyLog(
        state,
        enemy.id,
...
function buildEnemyIntentEffects(context: BattleContext,
  state: ExpeditionState,
  rng: Rng
): AtomicEffect[] {
    ...
    switch (enemy.kind) {
      case "brute": {
        ...
        intent = {
          ...
          targetId: target.id,
          value: enemy.attack,
          title: isEnemyFrenzied(context, state, enemy) ? "狂暴" : "攻击",
      ...
      case "charger": {
        if (enemy.chargeReady) {
          ...
          intent = {
            ...
            targetId: target.id,
            value: enemy.attack,
            title: isEnemyFrenzied(context, state, enemy) ? "狂暴重击" : "蓄力重击",
            description: `造成 ${enemy.attack} 点伤害`
          };
        } else {
          intent = {
...
export function completeNextRoundAfterFrenzyTransition(context: BattleContext,
  state: ExpeditionState,
  rng: Rng
): BattleTransition {
  if (state.mode.type !== "enemy-turn") {
  ...
  const dice = context.partyOrder.map((ownerId) => {
  ...
  const effects: AtomicEffect[] = [
    {
      ...
      dice,
      rerollsRemaining: context.catalog.balance.REROLLS_PER_ROUND
    },
    ...buildEnemyIntentEffects(context, state, rng)
  ];
  const resolved = resolveAtomicEffects(context, state, effects);
  return resolved.error
    ? { state, events: [], error: "command-not-available" }
...
export function beginNextRoundTransition(context: BattleContext,
  state: ExpeditionState,
  rng: Rng
): BattleTransition {
  const activation = activateScheduledFrenzyForNextRoundTransition(context, state);
  if (activation.error) return activation;
  const started = completeNextRoundAfterFrenzyTransition(context, activation.state, rng);

...
export function compileEffectRuntime(context: BattleContext,
  state: ExpeditionState,
  definitions: BattleEffectDefinitionRegistry = context.catalog.effects
...
export function mergeEffectResolutionOptions(context: BattleContext,
  state: ExpeditionState,
  options: EffectResolutionOptions
): EffectResolutionOptions {
  if (!hasActiveInstances(state)) return options;
  const compiled = compileEffectRuntime(context,
    state,
    options.effectDefinitions ?? context.catalog.effects
  );

...
export function resolveAtomicEffects(context: BattleContext,
  ...
  const effectiveOptions = mergeEffectResolutionOptions(context, input, options);
  ...
  while (queue.length > 0 && !error) {
    ...
    error = applyAtomicEffect(context, state, current.effect, emitter, effectiveOptions);
    if (error) break;
...
export function resolveEffectsCommand(context: BattleContext,
  input: ExpeditionState,
  actionLabel: string,
  effects: readonly AtomicEffect[],
  options: EffectResolutionOptions = {}
): EffectResolution {
  const resolution = resolveAtomicEffects(context, input, effects, options);

...
import { createLayerEnemies } from "../rules/enemies";
...
function createInitialState(context: BattleContext,
  location: string,
  ...
  return {
    ...
    enemySequence: 0,
    party: context.partyOrder.map((id) => ({
      id,
      hp: context.catalog.balance.MAX_HP,
      shield: 0,
      downed: false,
      rustLevel: 0,
      sealedNext: false
...
export function startLayerTransition(context: BattleContext,
  state: ExpeditionState,
  layer: number,
  rng: Rng,
  announce = true
  ...
  const enemies = createLayerEnemies(context, layer, () => {
    enemySequence += 1;
  ...
  const effects: AtomicEffect[] = [
    ...
    },
    {
      ...
      type: "append-log",
      tone: "gold",
      text: `进入第 ${layer} 层，收益倍率 ×${context.catalog.balance.LAYER_MULTIPLIERS[layer - 1]}。`
    }
  ];
    ...
    effects.push(
      {
        ...
        hp: context.catalog.balance.DOWNED_RETURN_HP
      },
      {
        ...
        type: "append-log",
        tone: "system",
        text: `${context.catalog.characters[member.id].name}在新一层完成重整，以 ${context.catalog.balance.DOWNED_RETURN_HP}/${context.catalog.balance.MAX_HP} 生命归队；命数锈蚀仍然保留。`
      }
    );
  }
  if (announce) {
  ...
  const entered = resolveAtomicEffects(context, state, effects);
  if (entered.error) {
    return { state, events: [], error: "command-not-available" };
  }
  const round = beginNextRoundTransition(context, entered.state, rng);
...
export function createExpeditionTransition(context: BattleContext,
  rng: Rng,
  location = "混沌领域",
  loadout = createEmptyBattleLoadout()
): BattleTransition {
  const state = createInitialState(context, location, loadout);
  return startLayerTransition(context, state, 1, rng, false);
}

export function createExpeditionState(context: BattleContext,
  rng: Rng,
  location = "混沌领域",
  loadout = createEmptyBattleLoadout()
): ExpeditionState {
  const transition = createExpeditionTransition(context, rng, location, loadout);
  transition.state.eventSequence = 0;
  return transition.state;
}

/** Legacy raw constructor: does not stamp external RNG metadata. New callers use createExpeditionFromSeed. */
export function createExpeditionStateFromInput(context: BattleContext,
  rng: Rng,
  input: BattleStartInput = {}
): ExpeditionState {
  return createExpeditionState(context,
    rng,
    input.location ?? "混沌领域",
...
export function goDeeperTransition(context: BattleContext,
  state: ExpeditionState,
  rng: Rng
): BattleTransition {
  ...
  return startLayerTransition(context, state, state.layer + 1, rng, true);
}

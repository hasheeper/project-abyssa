...
function getRng(
  ...
  dispatchOptions: BattleDispatchContext,
  streamName: RngStreamName
): CommandRng {
  const external = dispatchOptions.rng;
  if (external) {
    let draws = 0;
    return {
      rng: () => {
...
function dispatchSteal(context: BattleContext,
  state: ExpeditionState,
  command: Extract<BattleCommand, { type: "steal-from" }>,
  dispatchOptions: BattleDispatchContext
): BattleTransition {
  const random = getRng(state, dispatchOptions, "flavor");
  const result = performSteal(context, state, command.actorId, command.enemyId, random.rng);
  if (result.error) return rejected(state, result.error);
  random.commit(result.state);
  return result;
}

function resolveQueuedEnemy(context: BattleContext,
  state: ExpeditionState,
  emitter: EventEmitter
  ...
  state.eventSequence = emitter.startSequence + emitter.events.length;
  const transition = resolveEnemyIntentTransition(context, state, enemyId);
  if (transition.error) return null;
  appendTransitionEvents(emitter, transition);
  return transition.state;
}

function dispatchEndTurn(context: BattleContext,
  state: ExpeditionState,
  ...
  const random = getRng(state, dispatchOptions, "loot");

  const emitter = createEventEmitter(state, command);
  const prepared = prepareEnemyTurnTransition(context, state);
  ...
  while (
    ...
    const resolved = resolveQueuedEnemy(context, next, emitter);
    if (!resolved) break;
    next = resolved;
  }
  ...
  const recoil = applyFrenzyRecoilTransition(context, next);
  if (recoil.error) return rejected(state, recoil.error);
  appendTransitionEvents(emitter, recoil);
  const finished = settleEnemyTurnTransition(context,
    recoil.state,
    random.rng,
    closingHand
  );
...
export function dispatchBattleCommand(context: BattleContext,
  state: ExpeditionState,
  ...
  switch (command.type) {
    case "roll-dice": {
      const random = getRng(state, dispatchOptions, "combat");
      const transition = performInitialRoll(context, state, random.rng);
    ...
    case "reroll-dice": {
      const random = getRng(state, dispatchOptions, "combat");
      const transition = performReroll(context, state, random.rng);
      if (transition.error) return transition;
      random.commit(transition.state);
      return transition;
    }
    ...
    case "toggle-load": {
      return performToggleLoad(context, state, command.dieIndex);
    }

    case "attack-enemy":
      return performAttack(context, state, command.actorId, command.enemyId);
    case "block-intent":
      return performBlock(context, state, command.actorId, command.enemyId);
    case "heal-member":
      return performHeal(context, state, command.actorId, command.targetId);
    case "steal-from":
      return dispatchSteal(context, state, command, dispatchOptions);
    ...
    case "end-turn":
      return dispatchEndTurn(context, state, command, dispatchOptions);

    case "begin-enemy-turn": {
      return prepareEnemyTurnTransition(context, state);
    }

    case "resolve-next-enemy": {
      const enemyId = getQueuedEnemyId(state);
      return enemyId
        ? resolveEnemyIntentTransition(context, state, enemyId)
    ...
    case "next-round": {
      const random = getRng(state, dispatchOptions, "combat");
      const transition = beginNextRoundTransition(context, state, random.rng);
      if (transition.error) return transition;
      random.commit(transition.state);
      return transition;
    }
    ...
    case "go-deeper": {
      const random = getRng(state, dispatchOptions, "combat");
      const transition = goDeeperTransition(context, state, random.rng);
      if (transition.error) return transition;
      random.commit(transition.state);
      return transition;
    }
    ...
    case "leave-expedition": {
      const random = getRng(state, dispatchOptions, "loot");
      const transition = leaveExpeditionTransition(context, state, random.rng);
      if (transition.error) return transition;
      random.commit(transition.state);
      return transition;
    }

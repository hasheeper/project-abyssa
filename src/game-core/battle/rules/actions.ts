...
function checkActor(context: BattleContext,
  state: ExpeditionState,
  actorId: CharacterId
...
function resolveAction(context: BattleContext,
  state: ExpeditionState,
  label: string,
  effects: AtomicEffect[]
): NativeActionTransition {
  const resolved = resolveEffectsCommand(context, state, label, effects);
  return resolved.error
    ? fail(state, "invalid-target")
    : { state: resolved.state, events: resolved.events, error: null };
}

export function performAttack(context: BattleContext,
  state: ExpeditionState,
  actorId: CharacterId,
  enemyId: string
): NativeActionTransition {
  const actor = checkActor(context, state, actorId);
  ...
  const bounty = lethal
    ? Math.round(enemy.maxHp * 10 * context.catalog.balance.LAYER_MULTIPLIERS[state.layer - 1]!)
    : 0;
  const effects: AtomicEffect[] = [
    ...
    },
    {
      ...
      type: "append-log",
      tone: "good",
      text: `${context.catalog.characters[actorId].name}对${enemy.name}造成 ${actor.face.power} 点伤害。`
    }
  ];
  if (lethal) {
    effects.push(
  ...
  return resolveAction(context,
    state,
    `${context.catalog.characters[actorId].name}攻击${enemy.name}`,
    effects
  );
}

export function performBlock(context: BattleContext,
  state: ExpeditionState,
  actorId: CharacterId,
  enemyId: string
): NativeActionTransition {
  ...
  const effects: AtomicEffect[] = [
    ...
    },
    {
      ...
      type: "append-log",
      tone: "good",
      text: `${context.catalog.characters[actorId].name}为${context.catalog.characters[protectedId].name}架起 ${actor.face.power} 层盾牌。`
    },
    actionMarker(state, actorId, actor.face, target, "complete-action")
  ];
  return resolveAction(context,
    state,
    `${context.catalog.characters[actorId].name}格挡${enemy.name}`,
    effects
  );
}

export function performHeal(context: BattleContext,
  state: ExpeditionState,
  actorId: CharacterId,
  ...
  if (member.hp >= context.catalog.balance.MAX_HP) return fail(state, "target-full-hp");

  const target = { kind: "party-member" as const, id: targetId };
  const actualHealing = Math.min(context.catalog.balance.MAX_HP - member.hp, actor.face.power);
  const contribution = buildActionEffectContribution(context,
    state,
    "heal",
  ...
  const effects: AtomicEffect[] = [
    ...
    },
    {
      ...
      type: "append-log",
      tone: "good",
      text: `${context.catalog.characters[actorId].name}为${context.catalog.characters[targetId].name}恢复 ${actualHealing} 点生命。`
    }
  ];
  ...
  return resolveAction(context,
    state,
    `${context.catalog.characters[actorId].name}治疗${context.catalog.characters[targetId].name}`,
    effects
  );
}

export function performSteal(context: BattleContext,
  state: ExpeditionState,
  actorId: CharacterId,
  enemyId: string,
  rng: Rng
  ...
  const effects: AtomicEffect[] = [
    ...
    },
    {
      ...
      type: "append-log",
      tone: "gold",
      text: `${context.catalog.characters[actorId].name}对${enemy.name}顺手牵羊，囊袋 +${gain}G。`
    }
  ];
  if (foundTrinket) {
    effects.push({
  ...
  return resolveAction(context,
    state,
    `${context.catalog.characters[actorId].name}偷取${enemy.name}`,
    effects
  );
}

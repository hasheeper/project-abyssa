...
export function applyCombatEffect(context: BattleContext,
  ...
  switch (effect.type) {
    case "damage": {
      ...
      if (modified.target.kind === "party-member") {
        ...
        downPartyMember(context, state, member.id, emitter, effect);
      } else {
    ...
    case "heal": {
      ...
      const amount = Math.max(0, Math.round(modified.value));
      const hpBefore = member.hp;
      member.hp = Math.min(context.catalog.balance.MAX_HP, member.hp + amount);
      if (
        !emitter.emit(
          "healing-applied",
          {
    ...
    case "revive": {
      const member = state.party.find((candidate) => candidate.id === effect.target.id);
      if (!member || !member.downed) return "invalid-effect-target";
      member.hp = Math.max(1, Math.min(context.catalog.balance.MAX_HP, Math.round(effect.hp)));
      member.downed = false;
      emitter.emit(
        "unit-revived",
      ...
      if (effect.target.kind === "party-member") {
        ...
        if (effect.stat === "hp") {
          ...
          member.hp = Math.max(
            0,
            Math.min(
              context.catalog.balance.MAX_HP,

...
export function applyAtomicEffect(context: BattleContext,
  ...
  switch (effect.type) {
    ...
    case "modify-party-member":
      error = applyDiceEffect(context, state, effect, emitter);
      break;
    ...
    case "revive":
    case "modify-stat":
      error = applyCombatEffect(context, state, effect, emitter, options);
      break;

...
function migrateSchemaTwo(context: BattleContext, input: UnknownRecord): UnknownRecord {
...
function assertCurrentDto(context: BattleContext, value: unknown): asserts value is BattleSaveDto {
  ...
  const violations = collectExpeditionInvariantViolations(context,
...
export function migrateBattleSaveDto(context: BattleContext, input: unknown): BattleSaveDto {
  ...
  if (migrated.schemaVersion === 2) migrated = migrateSchemaTwo(context, migrated);
  if (migrated.schemaVersion === 3) migrated = migrateSchemaThree(migrated);
  assertCurrentDto(context, migrated);
  return structuredClone(migrated);
}

export function deserializeBattleState(context: BattleContext, serialized: string): ExpeditionState {
  const parsed: unknown = JSON.parse(serialized);
  return migrateBattleSaveDto(context, parsed).state;
}

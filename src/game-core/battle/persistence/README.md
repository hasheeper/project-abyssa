...
- `loot`：结算与奖励随机。
- `flavor`：当前旧版偷取等随机分支。名称不表示它只影响表现；偷取会改变金币，S1 保留既有分流行为。
...
S2 正式入口是 `createBattleEngine(validatedCatalog, routeId)` 的 create/restore/dispatch；此处旧 JSON 序列化继续服务兼容页面。正式应用存档及深层导入校验见 [应用层说明](/Users/liuhang/Documents/project-abyssa/src/game-application/README.md)。legacy 的 raw FromInput 仍不能替代正式 seed 构造器。

...
export function hasPendingAction(context: BattleContext, state: ExpeditionState): boolean {
...
export function getReadyDamage(context: BattleContext, state: ExpeditionState): number {
...
export function getPotentialDamage(context: BattleContext, state: ExpeditionState): number {
  return state.dice.reduce((sum, die) => {
...
export function assessStalling(context: BattleContext, state: ExpeditionState): StallVerdict {
  ...
  const partyPotential = getPotentialDamage(context, state);
  ...
  const grinding = state.stalledRounds >= context.catalog.balance.STALL_GRACE_ROUNDS;

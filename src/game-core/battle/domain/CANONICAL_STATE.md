...
本文定义旧 Battle schema v4 与临时执行状态的字段语义。S2 的正式应用存档按 Campaign / Expedition / Encounter 拆分；唯一所有权表在 [session/state.ts](/Users/liuhang/Documents/project-abyssa/src/game-core/session/state.ts)，应用层不保存第二份平坦 BattleState。下文 mode/result 规则仍适用于执行投影和旧兼容页面。

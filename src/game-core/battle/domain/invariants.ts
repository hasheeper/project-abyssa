...
export function collectExpeditionInvariantViolations(context: BattleContext,
  state: ExpeditionState
): ExpeditionInvariantViolation[] {
  const violations: ExpeditionInvariantViolation[] = [];
  const expectedOwners = [...context.partyOrder];
  ...
  for (const [index, member] of state.party.entries()) {
    ...
    if (!Number.isInteger(member.hp) || member.hp < 0 || member.hp > context.catalog.balance.MAX_HP) {
    ...
    const maxRust = context.catalog.characters[member.id].faces.filter(
  ...
  for (const [index, die] of state.dice.entries()) {
    ...
    const faceCount = context.catalog.characters[die.ownerId].faces.length;
    if (
      die.faceIndex !== null &&
  ...
  if (!Number.isInteger(state.layer) || state.layer < 1 || state.layer > context.catalog.balance.MAX_LAYER) {
  ...
  if (
    ...
    state.deepestLayer > context.catalog.balance.MAX_LAYER
  ) {
    add(
      violations,
  ...
  if (
    ...
    state.rerollsRemaining > context.catalog.balance.REROLLS_PER_ROUND
  ) {
    add(
      violations,
      "run.rerolls-range",
...
export function assertExpeditionInvariants(context: BattleContext, state: ExpeditionState): void {
  const violations = collectExpeditionInvariantViolations(context, state);

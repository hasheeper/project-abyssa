...
type ShopMetalCornerProps = {
  corner: "tl" | "tr" | "br" | "bl";
};

export function ShopMetalCorner({ corner }: ShopMetalCornerProps) {
  const prefix = `shop-metal-corner-${useId().replace(/:/g, "")}`;
  const id = (name: string) => `${prefix}-${name}`;

  return (
    <svg
      className="abyssa-shop-metal-corner"
      data-corner={corner}
      viewBox="0 0 216 198"

...
/** Shared artwork also lets navigation links keep native anchor semantics. */
export function RpgNotchedPillArt({ label, watermark, preserveAspectRatio, className }: Pick<RpgNotchedPillButtonProps, "label" | "watermark"> & {preserveAspectRatio?: string; className?: string}) {
  const watermarkOptions = resolveDiamondWatermark(watermark, { size: 26, outerOpacity: 0.72, innerOpacity: 0.62, innerInset: 6 });
  ...
  return (
    <svg className={className} viewBox="0 0 190 48" aria-hidden="true" preserveAspectRatio={preserveAspectRatio}>
      <defs>
        {watermarkOptions && <DiamondWatermark as="pattern" id={patternId} outerFill="var(--abyssa-notched-pattern-dark)" innerFill="var(--abyssa-notched-pattern-light)" patternTransform="translate(0 -2)" {...watermarkOptions} />}
      ...
      {watermarkOptions && <rect x="3" y="3" width="184" height="42" fill={`url(#${patternId})`} mask={`url(#${maskId})`} clipPath={`url(#${clipId})`} />}
      ...
      {label && <text className="abyssa-notched-pill__label" x="95" y="24">{label}</text>}
    </svg>
  );
}
...
export const RpgNotchedPillButton = forwardRef<HTMLButtonElement, RpgNotchedPillButtonProps>(
  function RpgNotchedPillButton(
    ...
    return (
      <button ref={ref} type={type} className={cx("abyssa-notched-pill", "abyssa-control-motion", className)} data-ui-motion={reduced ? "reduced" : "full"} data-variant={variant} data-selected={selected || undefined} aria-label={label} aria-pressed={selected === undefined ? undefined : selected} {...props}>
        <RpgNotchedPillArt className="abyssa-control-motion__art" label={label} watermark={watermark} />
      </button>
    );
  }
);

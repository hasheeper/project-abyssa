...
function PresentedModal({ onClose, title, children, className, panelClassName, side,
  dismissOnBackdrop = true, dismissOnEscape = true, returnFocusRef, onPresentChange, motionPreset = "surface" }: UiModalProps) {
  const [present, safeToRemove] = usePresence();
  const { reduced } = useUiMotion();
  const opacity = useMotionValue(0);
  const manor = motionPreset === "manor";
  const panelOpacity = useMotionValue(0);
  const offset = manor ? `0px -${manorWindowMotion.distancePx}px` : side === "right" ? `${motionTokens.surface.distancePx}px 0px` : `0px ${motionTokens.surface.distancePx}px`;
  ...
  useLayoutEffect(() => {
    ...
    const fade = animate(opacity, present ? 1 : 0, manor ? uiTransition(reduced ? motionTokens.surface.reducedMs : manorWindowMotion.scrimMs) : transition);
  ...
  }, [present, reduced, offset, opacity, translate, panelOpacity, manor, safeToRemove]);
  ...
  return <motion.div ref={rootRef} className={cx("abyssa-modal", className)}
    ...
    data-ui-motion={reduced ? "reduced" : "full"} style={{ opacity: manor ? undefined : opacity }}
    onMouseDown={event => { if (present && dismissOnBackdrop && event.target === event.currentTarget) onClose(); }}>
    {manor && <motion.div className="abyssa-modal__scrim" aria-hidden="true" style={{opacity}}/>}
    <motion.div ref={panelRef} className={cx("abyssa-modal__panel", panelClassName)} role="dialog"
      aria-modal="true" aria-label={title} tabIndex={-1} onKeyDown={handleKeyDown} style={{ translate, opacity: manor ? panelOpacity : undefined }}>

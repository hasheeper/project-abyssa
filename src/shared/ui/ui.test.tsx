...
import { characterStatusFixture } from "../testing/characterProfiles";
import { CharacterPortraitSelector } from "./patterns/CharacterPortraitSelector";
import { CharacterSelector } from "./patterns/CharacterSelector";
import { CharacterStatusScreen } from "./patterns/CharacterStatusScreen";
import { StatusPanel } from "./patterns/StatusPanel";
...
import { RpgBackButton } from "./primitives/RpgBackButton";
import { RpgCheckbox, RpgRadio } from "./primitives/RpgChoice";
import { RpgDialogue } from "./primitives/RpgDialogue";
import { RpgDiamondNode, RpgDiamondNodeTrack } from "./primitives/RpgDiamondNodeTrack";
import { RpgFrame } from "./primitives/RpgFrame";
import { RpgHeader } from "./primitives/RpgHeader";
import { RpgHexButton } from "./primitives/RpgHexButton";
import { RpgNotchButton } from "./primitives/RpgNotchButton";
import { RpgNotchedPillButton } from "./primitives/RpgNotchedPillButton";
import { RpgPanel } from "./primitives/RpgPanel";
import { RpgShapeButton } from "./primitives/RpgShapeButton";
import { RpgSquarePanel } from "./primitives/RpgSquarePanel";
import { RpgStatusNode } from "./primitives/RpgStatusNode";
import { RpgTab } from "./primitives/RpgTab";
  ...
  it("renders outfits, internal tabs and the centered character portrait selector", () => {
    ...
    const { container } = render(
      <CharacterStatusScreen
        characters={characterStatusFixture}
    ...
    expect(screen.getByRole("img", { name: "艾比希斯·贝尔泽兰头像" })).toHaveAttribute(
      "src",
      characterStatusFixture.find((character) => character.id === "abyssa")!.thumbnailUrl
    );
    expect(screen.getByRole("img", { name: "艾比希斯·贝尔泽兰原生质睡衣立绘" })).toHaveAttribute(
      "src",
      characterStatusFixture.find((character) => character.id === "abyssa")!.outfits![0]!.portraitUrl
    );
    ...
    expect(screen.getByRole("img", { name: "艾比希斯·贝尔泽兰哥特礼服立绘" })).toHaveAttribute(
      "src",
      characterStatusFixture.find((character) => character.id === "abyssa")!.outfits![1]!.portraitUrl
    );
    ...
    expect(screen.getByRole("img", { name: "薇薇安·桑格温角色立绘" })).toHaveAttribute(
      "src",
      characterStatusFixture.find((character) => character.id === "vivienne")!.portraitUrl
    );

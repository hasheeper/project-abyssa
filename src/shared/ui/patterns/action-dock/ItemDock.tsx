...
export type DockItem = {
  ...
  targets: readonly { id: string; label: string; onSelect: () => void; ref?: RefCallback<HTMLButtonElement> }[];
};
...
export function ItemDock({items, busy, onSelectionChange}: ItemDockProps) {
  ...
  return <section className="item-dock" aria-label="七槽道具坞" onKeyDown={e => {
    ...
    {item && <section ref={targetsPanel} className="item-dock__targets" aria-label={`${item.name}的使用目标`}>
      ...
      <div className="item-dock__target-list">
        {item.targets.map(target => <DiceActionButton ref={target.ref} key={target.id} label={target.label} disabled={busy || !!item.unavailableReason} onClick={() => {
    ...
    {item ? <div className="item-dock__spacer" aria-hidden="true"/> : <ol className="item-dock__slots" aria-label="携带道具">
      {Array.from({length:7}, (_, index) => {
        const supply = items[index];
        return <li key={supply?.id ?? `empty-${index}`} data-empty={!supply || undefined}>
          {supply ? <button type="button" ref={node => {if(node) buttons.current.set(supply.id,node); else buttons.current.delete(supply.id); supply.slotRef?.(node);}}

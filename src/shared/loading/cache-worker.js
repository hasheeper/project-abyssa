...
async function getAsset(asset, background = false) {
  ...
  if (!pending.has(key)) {
    const work = (async () => {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        ...
        try {
          const response = await fetch(new URL(asset.url, base), {cache: attempt ? 'reload' : 'force-cache', signal: controller.signal, priority: background ? 'low' : 'auto'});
...
async function prepare(background = false) {
  ...
  const step = async () => {
    while (cursor < GAME_ASSETS.assets.length) {
      ...
      try { if (!existing.has(keyFor(asset))) await getAsset(asset, background); loadedBytes += asset.bytes; }
      catch { failures.push(asset.url); }
      completed++;
      publish({type: 'progress', completed, total: GAME_ASSETS.assets.length, loadedBytes, totalBytes});
    }
  };
  await Promise.all(Array.from({length: background ? 2 : 6}, step));
...
self.addEventListener('message', event => {
  ...
  if (!preparation) preparation = prepare(event.data.background === true).finally(() => { preparation = undefined; });

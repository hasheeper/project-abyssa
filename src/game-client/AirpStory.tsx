...
import { DirectControls, DirectRecord, organizeDirectMemory } from "./airp-generation/DirectControls";
...
export function AirpStory() {
  ...
  const directTask = record?.schemaVersion === 4 ? record.airpDirect?.tasks.find(t => t.sceneId === scene.id) : undefined;
  if (directTask && (directTask.source === "undecided" || directTask.source === "requested")) return <DirectGenerationScene title={view.title} location={view.location} background={avgPresentation(scene.body).background}>
    <DirectControls sceneId={scene.id}/>
  ...
  const finishDirect = directTask?.source === "browser-direct" && (canTurnIn || directTask.task === "followup" && reading.node === scene.body.nodes.length - 1);
  return <AirpReading key={scene.id} sceneId={scene.id} wide title={view.title} location={view.location} background={avgPresentation(scene.source === "browser-direct" ? scene.body : parseAvgStory(scene.body)).background}
    lines={lines} cursor={lines.length - 1} busy={busy} finalLabel={finishDirect ? canTurnIn ? "确认交付并整理记忆" : "完成阅读并整理记忆" : canFinish ? "完成这件小事" : canTurnIn ? view.version === 1 ? "交付空药箱" : "确认交付" : reading.node === scene.body.nodes.length - 1 ? "完成阅读" : "下一句"}
    choice={node.kind === "choice" ? { id: `${scene.id}:${node.id}`, prompt: node.prompt, options: node.options } : null}
    canAdvance={node.kind !== "choice" && reading.node < scene.body.nodes.length - 1 && !canFinish && !canTurnIn}
    onAdvance={()=>session.dispatch({type:"airp-read",...base})} error={session.getSnapshot().error}
    onNext={async () => {
      const result = await session.dispatch(canFinish ? { type: "airp-finish", instanceId: instance.id } : canTurnIn ? { type: "airp-turn-in", instanceId: instance.id } : { type: "airp-read", ...base });
      if (result && finishDirect) await organizeDirectMemory(session, scene.id);
      return result;
    }}
    onChoose={async optionId => {
      if (!await session.dispatch({ type: "airp-accept", ...base, optionId })) throw Error("选择未能保存，请重试。");
    }}
    controls={instance.status === "offered" ? <>
      <ReadingTool label="稍后再说" caption="LATER" icon={returnIcon} disabled={busy} onClick={() => void session.dispatch({ type: "airp-defer", instanceId: instance.id })}/>
      <ReadingTool label="这次不接" caption="PASS" icon={forwardIcon} disabled={busy} onClick={() => void session.dispatch({ type: "airp-decline", instanceId: instance.id })}/>
    </> : <><span className="airp-saved">阅读进度自动保存</span>{directTask && <DirectRecord sceneId={scene.id} docked/>}<AirpOnlineControls docked>

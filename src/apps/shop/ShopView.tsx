...
const products: Record<Mode, ShopItem[]> = {
  buy: [
...
function resolveShopCategory(item: ShopItem): Exclude<ShopCategory, "all"> {
  if (item.id.startsWith("item.")) return "supplies";
...
function resolveItemIcon(item: ShopItem) {
...
export type LiveShop = {products: ShopItem[]; funds: number; crystals: number; busy: boolean; available: boolean; onPurchase: (id: string, quantity: number) => Promise<string | null>};
export function ShopView({live}: {live?: LiveShop}) {
  const catalog = live ? {buy: live.products, sell: [], appraise: []} : products;
  const [mode, setMode] = useState<Mode>("buy");
  const [selected, setSelected] = useState(defaults);
  const [previewStock, setStock] = useState(stockDefaults);
  const stock = live ? Object.fromEntries(live.products.map(p => [p.id, p.stock ?? 0])) : previewStock;
  const [previewLira, setLira] = useState(1247);
  const lira = live?.funds ?? previewLira;
  const [previewCrystals, setCrystals] = useState(8);
  const crystals = live?.crystals ?? previewCrystals;
  ...
  const item = useMemo(() => catalog[mode].find((entry) => entry.id === selected[mode]) ?? catalog[mode][0], [mode, selected, live?.products]);
  ...
  const unavailable = !!live && (live.busy || !live.available || quantity > remaining) || item.unavailable || remaining <= 0 || (mode === "appraise" && appraised.has(item.id));
  const description = mode === "appraise" && appraised.has(item.id) ? item.verdict ?? item.description : item.description;
  const filteredProducts = category === "all" ? catalog[mode] : catalog[mode].filter((entry) => resolveShopCategory(entry) === category);
  ...
  const categoryCounts = Object.fromEntries(shopCategories.map(({ id }) => [id, id === "all" ? catalog[mode].length : catalog[mode].filter((entry) => resolveShopCategory(entry) === id).length])) as Record<ShopCategory, number>;
  ...
  const selectItem = (id: string) => { const next = catalog[mode].find((entry) => entry.id === id); if (!next) return; setSelected((current) => ({ ...current, [mode]: id })); setQuantity(1); setLine(next.teaser ?? next.line); };
  const selectCategory = (next: ShopCategory) => {
    if (next === category || categoryCounts[next] === 0) return;
    const firstItem = next === "all" ? catalog[mode][0] : catalog[mode].find((entry) => resolveShopCategory(entry) === next);
    setCategory(next);
  ...
  async function transact() {
    if (live) {
      ...
      const error = await live.onPurchase(item.id, quantity);
      setLine(error ?? "收好啦~ 出门前记得把它放进随行的包里。使用剩下的，下次还能带走哦。");
      if (!error) setQuantity(1);
      return;
    }
    ...
    <div className="abyssa-shop-screen__shell">
      ...
      <section className="abyssa-shop-screen__main" aria-label="交易区">
        ...
        <RpgFrame className="abyssa-shop-detail-frame" padding="md"><div className="abyssa-shop-detail">
          ...
          <div className="abyssa-shop-detail__info"><div className="abyssa-shop-detail__head"><h3>{item.name}</h3><span className="abyssa-shop-detail__category">{item.category}</span></div><p className="abyssa-shop-detail__desc">{description}</p><div className="abyssa-shop-detail__meta"><span>{live ? "可补充" : "库存"}<b>{item.unavailable ? "—" : mode === "appraise" ? "∞" : remaining}</b></span><span>{mode === "appraise" ? "鉴定费" : mode === "sell" ? "回收价" : "单价"}<b data-currency={currency}>{item.unavailable ? "—" : reduced && item.price != null ? <><s><CurrencyAmount value={item.price} currency={currency} /></s><CurrencyAmount value={unit} currency={currency} /></> : <CurrencyAmount value={unit} currency={currency} />}</b></span></div><div className="abyssa-shop-detail__deal"><div className="abyssa-shop-qty" style={{ visibility: !item.unavailable && mode !== "appraise" ? "visible" : "hidden" }}><IconButton label="减少数量" icon="minus" size="sm" variant="dark" disabled={quantity <= 1 || !!live?.busy} onClick={() => setQuantity((current) => Math.max(1, current - 1))} /><output className="abyssa-shop-qty__value">{quantity}</output><IconButton label="增加数量" icon="plus" size="sm" variant="teal" disabled={quantity >= Math.max(remaining, 1) || unavailable} onClick={() => setQuantity((current) => Math.min(Math.max(remaining, 1), current + 1))} /></div><div className="abyssa-shop-detail__subtotal">{item.unavailable ? "备注" : mode === "appraise" ? "鉴定费" : "小计"}<b data-currency={currency}>{item.unavailable ? mode === "sell" ? "拒收" : "非卖品" : <CurrencyAmount value={mode === "appraise" ? unit : unit * quantity} currency={currency} />}</b></div></div></div>
        </div></RpgFrame>
        <RpgFrame className="abyssa-shop-screen__goldbar" padding="sm">
          ...
          <div className="abyssa-shop-screen__actions">
            {!live && mode !== "appraise" && <RpgNotchedPillButton label="砍 价" variant="dark" disabled={mode !== "buy" || currency !== "lira" || unavailable || reduced} onClick={haggle} />}
      ...
      <aside className="abyssa-shop-screen__side" aria-label="店主缇比">
        <RpgFrame className="abyssa-shop-screen__portrait" padding="none">
          ...
          <img src={tibbyPortrait} alt="缇比·奥雷利亚 上半身立绘" />

...
it("preserves select-then-activate behavior for pointer and keyboard", async () => {
  ...
  expect(onSelect).toHaveBeenCalledExactlyOnceWith("storage");
  expect(onActivate).not.toHaveBeenCalled();
  rerender(<MenuCommandDial selectedId="storage" onSelect={onSelect} onActivate={onActivate} />);
  await user.keyboard("{Enter}");
  expect(onActivate).toHaveBeenCalledExactlyOnceWith("storage");
});
...
it("keeps entry, feedback and transparent hit areas on separate planes", async () => {
  ...
  await user.hover(screen.getByRole("button", { name: /仓库/ }));
  expect(onSelect).not.toHaveBeenCalled();
  expect(onActivate).not.toHaveBeenCalled();
...
it("acknowledges selection and repeated confirmation without accumulating effect nodes", async () => {
  ...
  const storage = screen.getByRole("button", { name: /仓库/ });
  await user.click(storage);
  expect(storage).toHaveAttribute("aria-pressed", "true");
  expect(container.querySelector(".menu-dial__panel[data-command=storage] .menu-dial__response")).toHaveAttribute("data-kind", "select");
  expect(onActivate).not.toHaveBeenCalled();
  await user.keyboard("{Enter}");
...
it("treats a double click on an unselected command as select then one activation", async () => {
  ...
  render(<Controlled />);
  await user.dblClick(screen.getByRole("button", { name: /仓库/ }));
  expect(onSelect).toHaveBeenCalledExactlyOnceWith("storage");
  expect(onActivate).toHaveBeenCalledExactlyOnceWith("storage");
});

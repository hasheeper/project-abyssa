...
| `settings` | 设置界面概念预览。演出与显示两栏的模型及默认值对齐 `rp` 既有旋钮（打字机 step/dur、自动播放、过场、取景、动效开关），但修改只作用于本页预览，刷新即重置，尚未持久化或应用到其他入口；AI 服务栏为禁用占位。该 app 不导入 `rp`。 |

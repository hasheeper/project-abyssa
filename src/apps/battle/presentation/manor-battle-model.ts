import { expeditionEnemyArt } from "../../../content/presentation/expedition-art";
...
export function battleMemberName(member: Pick<DemoJourneyView["party"][number], "id" | "name"> | undefined, playerName?: string) {
  return member ? isPlayerActor(member.id) ? playerDisplayName(playerName) : member.name : "伙伴";
}
...
export function manorBattleModel(
  ...
  const enemies: BattleSurfaceEnemy[] = (battle?.enemies ?? []).map((e) => {
    ...
    const target = rawTarget && {...rawTarget, name: battleMemberName(rawTarget, playerName)};
    const memoryBoss = def.id === "enemy.memory.marietta";
    const clockwork = def.artId === "old-manor.clockwork-beast";
    const crossbow = def.id === "enemy.intro.crossbowman" || def.artId === "enemy.outlaw.crossbow";
    const title = bound
      ? "束缚"
      : i?.operation === "memory-reorder" ? "收线重排" : memoryBoss && i?.kind === "attack" ? "裁定" : i?.kind === "attack"
        ? def.behavior === "butler"
          ? "落幕"
          : def.behavior === "heiress" ? "举杯" : clockwork ? "钟鸣重击" : crossbow ? "射击" : "攻击"
        : i?.kind === "summon"
          ? "补席"
        : i?.kind === "seal"
          ? def.behavior === "butler" ? "门扉闭合" : "干扰"
          : i?.kind === "charge"
            ? clockwork ? "摆锤蓄势" : crossbow || view.tutorial?.runRef ? "装填" : "举盘"
            : i?.kind === "repair"
              ? "缝补"
              : "待命";
    const description = `${memoryBoss ? `侍偶护域：${e.protection ?? 0}。` : ""}${def.name} · ${title}${bound ? "：本回合无法行动" : i?.kind === "attack" ? target?.hp === 0 ? "：锁定目标已力竭，本轮落空" : `：对${target?.name}造成 ${e.damage} 点伤害${i.formula ? `（2＋${view.banquet?.guests ?? 0}位宾客，已格挡${i.blocked}）` : ""}` : i?.kind === "summon" ? `：${view.banquet?.reserve && view.banquet.guests < view.banquet.limit ? "补入一位候席客，下一回合行动" : "已满席或储备耗尽，本轮空过"}；剩余储备${view.banquet?.reserve ?? 0}` : i?.kind === "seal" ? `：下回合封锁${target?.name}的命数骰` : i?.kind === "charge" ? crossbow ? "：本轮装填，下回合射击" : "：蓄力，下回合重击" : i?.kind === "repair" ? "：为锁定的受伤同伴恢复 1 点生命" : i?.operation === "memory-reorder" ? "：调整侍偶邻接位置，既有出手队列不变" : "：暂时待命"}${options.some(o => o.choice === "attack" && o.targetId === e.id) ? `；本次实伤预览 ${options.filter(o => o.choice === "attack" && o.targetId === e.id).map(o => "damage" in o && o.damage && typeof o.damage === "object" && "applied" in o.damage ? o.damage.applied : o.amount).join("／")}` : ""}`;
    const art = expeditionEnemyArt(def);
    return {
      id: e.id,
      name: def.name!,
      hp: e.hp,

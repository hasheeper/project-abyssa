...
export type ExpeditionLedgerProps = {
  ...
  projected: number;
  battleGold?: number;
  currentLayerGold?: number;
  earthFactor?: number;
};

/** The drop-down contains records; live readings stay in the monitor. */
export function ExpeditionLedger({
  engine, layers = LAYER_MULTIPLIERS, memory, onClose, objective, handFactor, layerFactor, earthFactor = 1, projected, battleGold, currentLayerGold,
    ...
    <div className="abyssa-expedition-region abyssa-expedition-sidebar battle-ledger" aria-label="远征账本">
      ...
      <div className="battle-ledger__records">
        ...
        {!memory && <section className="battle-ledger__yield" aria-label="收益倍率明细">
          {!!battleGold && <p>本场拾取 <MoneyText value={battleGold}/>。</p>}
          ...
          {currentLayerGold === undefined
            ? <p>本层散金 <MoneyText value={engine.gold}/>，按当前倍率清层可得 <MoneyText value={projected}/>（四舍五入）。战斗中的读数只是预估，不是已经到账。</p>
            : <p>第 {engine.layer} 层已结算，入袋 <MoneyText value={currentLayerGold}/>。</p>}
          <p>每轮牌型加成在本层累积，清层后重置；已入袋的资金不再重复乘算，返馆领取后可花费。收益倍率不增加攻击伤害。</p>
        </section>}

...
test('page CSS is isolated without changing selector specificity, media rules or keyframes', async () => {
  ...
  assert.match(result.css, /:root:where\(:not\(\[data-game-page\]\), \[data-game-page="map"\]\)/);
  assert.match(result.css, /html:where\(:not\(\[data-game-page\]\), \[data-game-page="map"\]\)\[data-scene-transition\]/);
  assert.match(result.css, /:where\(html:not\(\[data-game-page\]\), html\[data-game-page="map"\]\) body/);

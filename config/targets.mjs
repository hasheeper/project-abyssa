...
export function resolveTarget(id) {
  ...
  const group = /** @type {Record<string, {kind: import('./types.js').EntryKind, port: number, home: string}>} */ ({
    game: { kind: 'game', port: 5190, home: 'index.html' },
    lab: { kind: 'lab', port: 5191, home: 'catalog.html' },
  ...
  return {
    id, entries: entry.kind === 'game' ? entries.filter(e => e.kind === 'game') : entryClosure([entry.id]), home: entry.html,

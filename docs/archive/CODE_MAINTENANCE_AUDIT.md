> 历史档案：2026-09-07文档整理时归档。原路径：`docs/CODE_MAINTENANCE_AUDIT.md`。保留当时的设计、状态和验证证据，不作为当前排期或默认机制；当前入口见[文档索引](../README.md)。
...
- 地图底图和三个节点图已于 2026-09-05 归入 `src/assets/map/terrain/` 与 `src/assets/map/landmarks/`，见[地图素材说明](../../src/assets/map/README.md)；骰局/商店原型中的部分 Tibby 立绘仍来自 `files.catbox.moe`；

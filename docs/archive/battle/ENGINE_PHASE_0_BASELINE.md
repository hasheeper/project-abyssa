> 历史档案：2026-09-07文档整理时归档。原路径：`src/apps/battle/ENGINE_PHASE_0_BASELINE.md`。保留当时的设计、状态和验证证据，不作为当前排期或默认机制；当前入口见[文档索引](../../README.md)。
...
S1 迁移说明（2026-09-05）：本文保留旧环境、路径与命令作为历史记录。当前规则和基线测试已迁入 `src/game-core/battle`，请使用 [当前维护指南](/Users/liuhang/Documents/project-abyssa/src/apps/battle/DEVELOPMENT_GUIDE.md) 中的命令；核心验收禁止用 `UPDATE_BATTLE_BASELINE=1` 跳过断言。

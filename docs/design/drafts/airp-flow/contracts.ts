/** Design handoff only. Not imported by the game. Authored by the coordinating agent. */
...
export type FlowDisplay =
  ...
  | { phase: "closing"; key: string; next: { kind: "background" } | { kind: "open"; key: string } | { kind: "reader"; key: string } }
  | { phase: "reader"; key: string };
...
export function flowDisplayReducer(state: FlowDisplay, event: FlowDisplayEvent): FlowDisplay {
  ...
  if (event.type === "read" && state.phase === "open" && state.key === event.key)
    return { phase: "closing", key: state.key, next: { kind: "reader", key: state.key } };

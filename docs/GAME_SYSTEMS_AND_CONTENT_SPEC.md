...
> 更新：2026-09-15；版本：1.4。范围：当前工作树的正式默认游戏，以及与它相邻的原型／未实现功能。
>
...
**工程上的游玩闭环已接入，CG序幕的视觉、阅读与存档链路已阶段收口；洋馆首晨与四场岩窟教学已接入，内容12已导入六场用户定稿与诺玛事件教程，敌图仍为占位，战斗压力与现有剧作尚未达到Demo交付标准。**

| 范围 | 当前实际情况 |
| --- | --- |
| 当前默认包 | `abyssa.demo`，`contentVersion: 12`，`rulesVersion: 4`；应用记录／协议为4；四战＋事件五房教学，四型AIRP见独立实施记录 |
| 已可循环游玩 | 建档、角色检视、地图编队、配给、庄园初战／维护、事件、结算、回馆、成长／装备、商店补充、再次出征 |
| 已可完成角色开放 | 庄园首通→玛丽埃塔记事顶部回忆→刻仪兽战→当下收束→玛亲征 |
| 战斗操作指引 | 左侧菜单可手动开启共用高亮／短说明，跟随真实骰子和行动；不改变原布局，不记为初章完成，四场岩窟已复用此画布自动引导 |
| 当前新档入口 | 标题“新的开始”先弹起点选择，选择前不建档／不播CG。完整开始：四幕15张CG→Logo→洋馆S1／S2（默认AVG／可切RP，四组三选一＋一组二选一）→章节交接→岩窟四战＋E1→返馆AVG→一次领奖→自由枢纽。另可跳过序章／第一章／教程，见下文「新建起点选择」 |
| 尚无 | 序幕音频／独立动作帧、S2-6行军外景及地图切面、正式岩窟敌图；第一章六场正文已有用户定稿 |
...
教学发布基线[内容7](../src/content/gameplay/demo-v7/content.ts)继承内容6首晨并增加四场教学，内容9保留原四房；当前默认12经11继承离线卡池并使用新版五房教学。首晨120节点、五处选择原样保留，最后一处仅A／B。内容5／6原档可按来源证明续接、保留原档和已选分支；运行时计算并验证摘要，不能仅用版本号接受导入。装配证据：[player-runtime](../src/game-runtime/player-runtime.ts)、[首晨实施记录](design/FIRST_MORNING_IMPLEMENTATION.md)。

09-13 G2新增[显式内容11](../src/content/gameplay/demo-v11/content.ts)：四战＋E1五房、固定后段种子、权威教学步骤／事件引用、重试与一次领奖；继承内容9，不带内容10在线协议。**G3原UI已接入，默认新建档切到11，不提供旧档迁移到11**。事件选择一人，独立抽面；0费用／0奖励，不执行战斗治疗。带做战3首轮真实两对／诺铭约后释放，Boss不加教学门禁。详细契约见[G2记录](audits/2026-09-13-tide-guided-g2.md)，原控件、事件结果和入口切换见[G3记录](audits/2026-09-13-tide-guided-g3.md)。

09-15以内容12隔离发布六场定稿：S3-1～S3-5、S4-1，各三屏共78帧，按用户最新确认只显示中文，不展示日文或双语括注，不再播放S4-2。E1改为诺玛，独立事件种子7，真实抽出face.norma.01；两段战斗种子及36G＋8G一次结算不变。默认新档改用12，历史11摘要／旧稿／艾洛拉事件保持原样，不自动迁移。归来面板改为“洋馆记事”入口，按归来、同伴、整备分栏，账目仍来自实际已提交记录。

## 2. 现在实际能走的完整流程

```mermaid
flowchart TD
  A[标题：新建／继续／选档] -->|新的开始| A0[起点弹窗：关闭不建档]
  A0 -->|完整开始| P{序幕未完成？}
  A0 -->|跳过序章| R
  A0 -->|跳过第一章| T0
  A0 -->|跳过教程| D
  ...
  R -->|S2出门及黑场章名| T0[S3-1雾滩进场剧情]
  T0 -->|读完或跳过本段| TM[战斗与探索规则总览]
  TM -->|玩家点击开始战斗| T[退潮岩窟四战＋E1与战间AVG]
  T --> TR[返馆AVG与一次领奖]
  TR --> K
  O -->|否| B{有活动远征或回忆？}
  B -->|有| C[恢复对应战斗／剧情]
...
- 完整开始先播放序幕及首晨，再进入四场岩窟与E1。正常通关需领取返馆报酬后才开放自由经济；新档主动跳过教程与旧冒险档兼容豁免是不同来源，不记为通关。
...
**O3-T已接通：** 序幕及首晨→四场教学→返馆→自由枢纽与庄园。首掷seed8267、检查点和一次领奖见[正式契约](plans/TIDE_CAVE_RULES_AND_SAVE_CONTRACT_V0_1.md)；浏览器验证与剩余制作项见[O3-T记录](audits/2026-09-09-tide-cave-o3-closeout.md)。正式敌图、文案与真人配平尚未验收。
...
| 序幕 | 完整开始为`A1-01 / playing`；逐镜提交，不改变世界时间／资金；跳过起点将其记为`skipped` |
...
| 弹窗选项 | 实际入口 | 存档语义 |
| --- | --- | --- |
| 完整开始 | 序章首镜 | 序章、首晨未读；教程待开始 |
| 跳过序章 | 洋馆首晨第0页 | 仅序章`skipped` |
| 跳过第一章（抵达教程） | S3-1 雾滩进场剧情 | 序章、首晨`skipped`；自动建立教程远征，播放 S3-1，读完后显示总览，确认后进入战 1 |
| 跳过教程 | 自由枢纽 | 序章、首晨`skipped`；教程`exempt / player-skipped` |

这些是递进起点，不是三个独立开关。用户选择前不创建存档；取消不写入。四种起点都写`game-start-selected`事实，仅新内容11的原生新档第1次提交可用，不能用于跳过已进行的战斗或改写旧档。跳过不代选剧情分支、不生成战斗／货物／领取记录，资金仍0、时间仍第1天晨；普通出征沿用原基础配给。创建与起点提交分别幂等，失败重试沿用同一档案身份。刷新／继续／完整备份恢复读取同一权威记录，显式在线10入口不变。

内容11在首晨结束或快捷抵达教程后，顺序为 **S3-1 进场剧情 → 战斗与探索规则总览 → 战 1**。手册为五章13节，战前总览与实战弹层共用同一阅读器，默认打开「界面与操作」。该页用真实战斗图标注敌人与意图、队员卡、行动骰、回合操作栏、进度与反馈、菜单与帮助六个区域，再以五步完整教程解释观察意图、投掷、固定／重掷、选人和目标、结束回合各自的操作及结果。操作栏、敌方头顶意图、队员卡与骰子的对应关系分别裁切放大。不能仅以按钮名称或箭头替代操作说明。

| 章 | 小节 | 规则归属 |
| --- | --- | --- |
| 战斗与地牢 | 界面与操作；远征流程；回合顺序与撤回；生命与力竭 | 界面定位与操作入口、四级层级、五步结算时序、撤回边界、生命与归队 |
| 骰子 | 战面与命数；力度与战面；命数、花色与铭文；行动流程与骰子状态 | 双层识别、动作目标、角标、操作及状态 |
| 倍率机制 | 牌型机制：回合末自动凑牌；总倍率与金币结算 | 牌型天梯、三项倍率、散金／入袋／钱包、通关与团灭结算 |
| 其他机制 | 事件判定；道具与补给 | 事件通用设计：人数由方案决定、角色各自贡献骰面、战面优先与命数汇总；七种补给的效果与使用条件 |
| 角色词条 | 角色能力与配置 | 只区分骰面动作、铭约、编队效果、装备和成长；具体条款查角色档案 |

采用可操作的教程文体：先讲玩家做什么、界面怎样响应、结果怎样接到下一步，再以规则表供查阅。去重不等于极简；「远征流程」以完整五步串起出发、房间战斗、战间整备、清层和返馆，解释生命与补给如何跨战延续、散金何时变为入袋金币，最后保留进度速查表。道具仍用一页一张表，不再分成携带／治疗／战术品三节；具体金币损失比例留在倍率章，生命页不混入经济账本。角色部分只留通用机制，不枚举角色、阵容、专属技能或开放状态；大地倍率条件归倍率页，基础角标页不引入编队细则。归并与验收记录见[手册内容审计](audits/2026-09-13-handbook-content-audit.md)。

骰面复用真实 `ExpeditionFlatDieFrame`：完整骰面分实战／结算两层；力度保留五个右下角刻痕局部和六种战面图；命数／花色／品质保留三行放大角标。空面使用大叉，与命数沉眠分别呈现。操作五步后直接接六状态速查，不再插入「核心规则」。牌型页严格保留三条规则与 `tables.hand-patterns` 的十行三列表；金铭／两对真实读数图归入总倍率页，不另设算例目录。道具原图标合入对应表格行；事件、牌型预览与倍率使用标注局部，不用全屏截图代替图解。较长页面保留“向下查阅 ↓”。

回合时序为意图公开、队伍行动、牌型结算、铭约触发、敌方结算。撤回恢复当前行动阶段的操作与资源；成功 ROLL／REROLL 清空此前历史，END TURN 或击败最后一个敌人关闭撤回窗口。铭约独立触发，包含攻击、治疗与阵位等效果。文案按现行 v4 规则编写，本次仅整理展示，不修改玩法、存档或 RP 后端。

抵达教程时由原 `start-expedition` 建立固定队伍、配给和种子，先播放 S3-1。读完或跳过该段对白后显示总览，此时权威进度仍停在 S3-1 的战前边界，战斗不可操作。点击“开始战斗”才提交原 `tutorial-read`，进入第一场战斗；存储失败沿用原请求重试，不重新出征。总览是同标签页的阅读位置，刷新或返回标题后继续会恢复；新标签页可以重读剧情，不制造通关事实。已进入战斗的档案不重复弹出总览。旧7～10教程保持原流程。战斗／事件菜单的“玩法手册”打开时使战斗主界面 inert、暂停教程高亮并禁止菜单提交，关闭后恢复选中队员、事件对象与提示；不提交游戏命令，也不等于“退出带做”。

实际文案源为 [`src/content/presentation/tutorial/handbook.json`](../src/content/presentation/tutorial/handbook.json)，战前总览与实战弹层直接导入，修改后随前端构建发布，不存入 RP 或玩家事实库。旧 `docs/TUTORIAL_COPY_EDITABLE.json` 是未回填的历史编辑副本，不是实时源。表格可嵌入同一小节的语义分组；布局沿用固定舞台、目录与阅读区。总览与手册弹层在自身作用域固定原冷灰文字、青色强调和边框色，不继承战斗／标题皮肤的暖金文字变量。局部素材来源见[截图说明](../src/assets/tutorial/README.md)。

事件页说明统一设计，不再把已接入的单人整理写成全体事件的固定规则：所选处理方式规定参与人数和要求；各角色贡献自己的骰面，先核对战面匹配数，不满足时再按方案计算清醒命数。移除通用页中的固定支援面清单、≥4门槛和统一扣费时点；按最新文案分为强成功、弱成功、失败三种结果。单人／多人表只说明参与方式与判定范围，不重复判定步骤。现行玩法仍为§6.3的单人事件，多人协议仍见[待实施方案](plans/MULTI_ACTOR_DICE_EVENT_RULES_AND_SAVE_CONTRACT.md)；该旧方案的候选第四档未纳入本次教材，本轮没有启用多人操作或改写存档。
...
内部清层计算为：`四舍五入〔散金 ×（1＋本层累计牌型加成，最多2）×层深倍率×大地加成〕`。五层倍率依次1、1.25、1.5、1.75、2；大地为1.1，否则1。清层后散金与牌型累计归零。
...
| 页面／模块 | 正式可用 | 当前边界 |
| --- | --- | --- |
| 标题／序幕 | 多档、新建、继续、导入导出、复制升级／二周目；四幕CG、阅读与逐镜恢复 | 岩窟正式文案／敌图待补；序幕音频与动作分层待补；标题设置命令未接通完整设置页 |
| 枢纽 | 共享资金／进度、前往地图／洋馆／商店／角色等 | 不是已经实现完整任务／成就／图鉴系统 |
| 地图 | 真实可用名册、配给选择、冻结配置并发起远征 | 正式当下只接庄园；托管、其他主题地牢没有生产玩法 |
| 洋馆 | 单张景致与角色检视、共享余额／补给、“洋馆记事”的归来／成长／回忆入口；合法馆内状态可手动推进相位，真实保存一次 | 建造、修缮、升级、生产、收取未开放，设施等级仍为展示数据 |
...
| `abyssa.demo` 内容6 | 4 | 保留完整S1／S2及五组选项旧档 |
| `abyssa.demo` 内容7 | 4 | 岩窟四战、教学、返馆与一次领奖基线 |
| `abyssa.demo` 内容8 | 4 | 单条药箱手写委托与本地恢复 |
| `abyssa.demo` 内容9 | 4 | **当前默认**；继承教学，四型8张工作稿与时局 |
| `abyssa.demo` 内容10 | 4 | 显式联机新档；仅药箱归来与后续，不自动迁移旧档 |
| `abyssa.demo` 内容11 | 4 | G3新建档默认五房教学；继承9，guide版本1，原UI已接入，不支持旧档迁移到11 |
...
- 序幕完成／跳过由正式进度事实证明，旧档不强制重播；新周目重新开场且可显式跳过。内容7教学承接已实现：旧首晨保留游标与选择，已有冒险经历豁免；现已完成前端接线并注册。
...
Abyssa现有玩法、初章与成长不依赖LLM在线。AIRP直接调用rp通用应用后端：制作与运行同定义、独立Session，Abyssa负责接纳与呈现。显式内容10接入药箱归来／后续，默认12经11继承9的离线卡池。当前0.3.2已安装，按用户更正小模型为deepseek-flash，Writing收紧事实承接；4来源／11摘录、阶段隔离及原预算保留。版本、玩家链、恢复和浏览器隔离验证通过，两情境四场真实闭环／摘要召回／重放／AVG通过，文稿事实边界与节拍仍需修订，见[0.3.2记录](audits/2026-09-12-airp-032-acceptance.md)。0.2.1完整真实基线及0.3.1的失败记录另留，不冒充本轮通过。未开放修订动作、扩容量或迁移旧档，不建设内容发布器或浏览器内核，见[AIRP-4计划](plans/AIRP_4_APPLICATION_INTEGRATION_PLAN.md)。

[变量与记忆](plans/AIRP_4_APPLICATION_INTEGRATION_PLAN.md#5-变量与记忆)复用rp状态／权限／检查点，通用机制归rp、AIRP定义记忆策略。游戏已有Fact来源与知情筛选，暂不迁移整个Campaign；玩法事实由现有命令持有，rp叙事State不重复结算。最后一条实际阅读产生确认ticket，required Updater写入受控摘要，模型候选不等于玩家经历。原身份备份重放、迟到结果拒绝、降级与同origin双窗口恢复已验；在线档新身份复制、自动Fork和跨设备强幂等暂不支持。

AIRP已形成[勘探报告](audits/2026-09-09-airp-narrative-exploration.md)、[DEMO实施计划](plans/AIRP_NARRATIVE_DEMO_PLAN.md)、[AIRP-1契约](plans/AIRP_1_CONTRACTS_AND_FIRST_ERRAND.md)与[AIRP-2实施记录](plans/AIRP_2_IMPLEMENTATION.md)。内容8接入首条“旧药箱的搭扣”：第三层真实房间证据、成功带回、手写归来对白、一次共同记忆、冷却及原身份备份恢复。验证结果与版本修订以实施记录为准。[AIRP-3](plans/AIRP_3_IMPLEMENTATION.md)以内容9新增四型共8张卡、三条真实出击链、每型一个活动事件、双轨失效、备用再提供与64日主题冷却。牵线必须提交指定角色的专属对话，家务／小景须明确完成；所有奖励仅为记忆，无新增货币或好感。内容8／9仍是本地玩法与手写稿，不放宽其三情绪、短场景与handwritten校验。[AIRP-4](plans/AIRP_4_IMPLEMENTATION.md)在内容10通过独立在线契约接入14情绪、文本与可见创作记录；模型不生成选项／跳转／奖励，正文经接纳后冻结，旧稿与已有存档不自动改写。

仓库登记19个Vite入口（10个game分类、4个lab、5个tool）；构建分类不代表每个入口都已接正式Campaign。Node要求见`.nvmrc`（22.23.2），运行与构建见[工程入口](../config/README.md)。

原总览曾导出已校验Catalog核对规则；09-12先复核版本，再实施客户端兼容与药箱资料接线并隔离验证，不重新宣称完成全套玩家E2E或真实内容验收。历史验证记录按日期与对应版本使用。

## 12. 已知缺口与接下来的顺序

游戏优先级统一见[Abyssa DEMO推进计划](plans/ABYSSA_DEMO_NEXT_STEPS.md)。rp通用基底足以开工，rp的配置界面改造不是本项目依赖；应用版本不匹配与药箱专用限制须由游戏接线处理。下表保留游戏缺口，不重新启动已完成的O2／O3工程。

| 优先级／范围 | 现状与下一结果 |
| --- | --- |
| 当前先做：初章 | 内容12六场中文单语定稿／诺玛E1／S4-1收束及组件化记事已接通，下一步G5真人试教；序幕音频／分层另补 |
| 庄园与刻仪兽压力 | 基础应对过于稳定；撤离、补给与机制取舍缺乏重量，需要重新设计敌群／意图与资源曲线 |
| 剧作 | 成长过多重复留座、吃饭、休息；事件提前解释答案；回忆未充分建立玛的变化与亲征因果，需要分场返工 |
| 教学接入契约 | 内容7及后续包已接入独立单层四场、故事／重试及幂等领奖，明确排除庄园成长奖励；旧庄园档保持兼容 |
| 多人事件 | 规则与存档提案存在，正式玩法未实现；不计入本轮初章已完成功能 |
| AIRP叙事闭环 | 内容11继承9的四型离线卡池，内容10显式在线；0.3.2已安装，改用deepseek-flash并收紧Writing，隔离回归通过，两情境四场真实闭环／摘要召回／重放／AVG通过，文稿事实边界与节拍仍需修订。过关后再扩四型，旧真实／模拟证据分开保留 |
| 美术与性能 | 已有主场景、立绘、皮肤与动效；缺姿态和事件配图仍需补，完整人工与最大负载表现验收待齐 |
| 后续系统 | 完整经营、额外装备／高等级、其他三地牢、托管、随时逃跑及超出AIRP首版范围的长期LLM日常能力均单独排期 |

09-07记录的内容3 Catalog对照模拟共96条：基础应对初战15/16、维护16/16、回忆16/16；仅主动攻击初战4/16、维护15/16、回忆16/16。基础应对进入管家16/16满血、进入千金14/16满血。这些是指定策略／种子的证据，不是玩家通关率，不能概括为所有模式随机乱点都能赢。详见[质量审计](audits/2026-09-07-demo-experience-quality.md)。

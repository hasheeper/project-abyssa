...
> 更新：2026-09-07；版本：1.0。范围：当前工作树的正式默认游戏，以及与它相邻的原型／未实现功能。
>
...
**工程上的游玩闭环已经建立；初章尚未接入，战斗压力与剧作尚未达到 Demo 交付标准。**

| 范围 | 当前实际情况 |
| --- | --- |
| 正式默认包 | `abyssa.demo`，`contentVersion: 3`，`rulesVersion: 4`；应用记录／协议为4 |
| 已可循环游玩 | 建档、角色检视、地图编队、配给、庄园初战／维护、事件、结算、回馆、成长／装备、商店补充、再次出征 |
| 已可完成角色开放 | 庄园首通→玛丽埃塔记事顶部回忆→刻仪兽战→当下收束→玛亲征 |
| 当前新档入口 | 标题“新的开始”创建真实档案，然后直接进入枢纽菜单 |
| 尚无 | 正式开场 CG 叙事、洋馆初介绍、基础操作教程、强盗／经典小怪教学副本 |
...
当前内容引用摘要：`8b0267560ab2037ce862de5d3f2bbe30e2b252b5bbdffddb19a5d505ad0f9080`。装配证据：[player-runtime](../src/game-runtime/player-runtime.ts)、[当前内容](../src/content/gameplay/demo-v3/content.ts)。

## 2. 现在实际能走的完整流程

```mermaid
flowchart TD
  A[标题：新建／继续／选档] --> B{有活动远征或回忆？}
  B -->|有| C[恢复对应战斗／剧情]
  B -->|无| D[枢纽菜单]
  D --> E[角色页：配置／记事／装备]
  D --> F[地图：选可用伙伴与至多4类补给]
...
- 新档无初章，玩家目前会直接面对菜单、地图和庄园。菜单不是开场剧情。
...
**新增目标，尚未实施：** 新的开始→CG／简短世界介绍→洋馆介绍→小钩子与基础教程→简单初始副本→回馆，再承接自由枢纽与庄园。已确认方向与具体建议见[初章计划](plans/DEMO_PROLOGUE_AND_ONBOARDING_PLAN.md)。
...
| 项目 | 当前值／规则 |
| --- | --- |
| 角色范围 | 凯尔、尤斯缇丝、艾洛拉、柯萝萝、诺玛、玛丽埃塔六人有正式战斗规则 |
| 初始可操作 | 凯尔＋尤／艾／柯／诺；玛在洋馆履职，但亲征未开放 |
| 队伍 | 核心允许凯尔＋至多四名已开放伙伴；最大五人，不是六人同时上场 |
| 生命 | 现行六人均为最大3HP；每次普通出征以满血开始 |
| 成长 | 初始 Lv.1、无已领成长；上限 Lv.3；凯尔走团队里程碑 |
...
下表是 **Lv.1**。`金／锈`是品质，未标为素；`眠`是命数沉眠，未标为清醒。沉眠不禁止行动，只排除成牌。凯尔面6是唯一命点万能；诺玛面1只在攻／挡／治之间选择，点数仍是1。

| 角色 | 面1 | 面2 | 面3 | 面4 | 面5 | 面6 |
| --- | --- | --- | --- | --- | --- | --- |
| 凯尔 | 攻1·锈可除 | 攻1 | 挡1 | 护卫2，护别人3 | 治1 | 攻／挡／治1·金，命点万能 |
...
| 角色 | 初始醒／眠 | 面1→6的花色 | 定位 |
| --- | --- | --- | --- |
| 凯尔 | 6／0 | 尘／尘／圣／圣／圣／圣 | 补位、保护、唯一命点万能 |
...
只结算最佳牌型，不把多种牌型加成叠加。用于该牌型的每枚金面 +0.10、锈面 −0.10，素面不变；散牌不凭品质单独产生收益，加成最低为0。比较顺序先基础牌型，再品质，再点数组合；凯尔万能在1–6中按同一顺序选择，并供该轮铭约共用。
...
| 凯尔团队里程碑 | 五位成长角色中任意两人已达Lv.3，护卫面品质一次性变金；基础护卫数值不变 |

现有十个成长事件（尤／艾／柯／诺／玛各两段），可稍后继续，结果须明确完成才领取。初始版没有Lv.4／5、连续刷好感经验、全员统一升级或凯尔晶石除锈获取线。
...
- 每人只有一个实际可装卸的通用槽；适用者是尤／艾／柯／诺。凯尔与玛没有原生空面，不能装备这两件。
...
| 页面／模块 | 正式可用 | 当前边界 |
| --- | --- | --- |
| 标题 | 多档、新建、继续、导入、导出、坏档诊断、符合条件的复制升级／二周目 | 尚无初章；标题设置命令未接通完整设置页 |
...
仓库登记18个Vite入口（9个game分类、4个lab、5个tool）；构建分类不代表每个入口都已接正式Campaign。Node要求见`.nvmrc`（22.23.2），运行与构建见[工程入口](../config/README.md)。
...
| 优先级／范围 | 现状与下一结果 |
| --- | --- |
| 当前先做：初章 | 世界CG、洋馆介绍、简单钩子、教学副本尚无；先交分镜／分场与教学卡，再收口持久阶段、奖励和旧档策略 |
